...
public:
  ...
  static void DeserializeMap(FString json, TMap<FString, FString> &map) {
    ...
    if (FJsonSerializer::Deserialize(JsonReader, JsonParsed) &&
      ...
      for (const auto &JsonEntry : JsonObject->Values) {
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FEpicLeaderboardGetEntriesEvent,
...
UCLASS()
class EPICLEADERBOARD_API UGetLeaderboardEntries
...
public:
  ...
  static UGetLeaderboardEntries *
  GetLeaderboardEntries(FEpicLeaderboardGame Game, FEpicLeaderboard Leaderboard,
                        FString Username,
                        Timeframe Timeframe = Timeframe::AllTime,
                        bool AroundPlayer = true, bool Local = false) {
...
protected:
  virtual void Activate() override {
    ...
    FString BaseURL =
        FString(TEXT(SERVER_URL)) + FString(TEXT("/api/getScores"));
    ...
    QueryParams.Add(TEXT("timeframe"),
                    FString::FromInt((int32)LeaderboardTimeFrame));
    ...
    FString FullURL = BaseURL + "?" +
                      EpicLeaderboardInterface::ConstructParamsURL(QueryParams);

    // setup the request
    TSharedRef<IHttpRequest> Request = FHttpModule::Get().CreateRequest();
    Request->OnProcessRequestComplete().BindUObject(
        this, &UGetLeaderboardEntries::OnHighscoreResponseReceived);

    Request->SetURL(FullURL);
    Request->SetVerb("GET");
    Request->SetHeader(TEXT("User-Agent"), "X-EpicLeaderboard UE5");
  ...
  void OnHighscoreResponseReceived(FHttpRequestPtr Request,
    ...
    // UE_LOG(LogTemp, Warning, TEXT("GetLeaderboardEntries Response Time: %s"),
    // *time);
    ...
    if (FJsonSerializer::Deserialize(JsonReader, JsonParsed) &&
      ...
      TSharedPtr<FJsonValue> *topScores =
          JsonObject->Values.Find(TEXT("scores"));
      ...
      // parse player score
      TSharedPtr<FJsonValue> *playerScore =
          JsonObject->Values.Find(TEXT("playerscore"));

      if (playerScore != nullptr && playerScore->IsValid()) {
        TSharedPtr<FJsonObject> PlayerScoreObject = playerScore->Get()->AsObject();
        if (PlayerScoreObject.IsValid()) {
          FJsonObjectConverter::JsonObjectToUStruct<FEpicLeaderboardEntry>(
              PlayerScoreObject.ToSharedRef(), &PlayerEntry, 0, 0);

          // deserialize metadata
          EpicLeaderboardInterface::DeserializeMap(PlayerEntry.Meta,
...
UCLASS()
class EPICLEADERBOARD_API USubmitLeaderboardEntry
    : public UBlueprintAsyncActionBase {
  GENERATED_BODY()
...
public:
  ...
  static USubmitLeaderboardEntry *SubmitLeaderboardEntryWithMetadata(
      FEpicLeaderboardGame Game, FEpicLeaderboard Leaderboard, FString Username,
      double Score, TMap<FString, FString> Metadata) {
    USubmitLeaderboardEntry *BlueprintNode =
        NewObject<USubmitLeaderboardEntry>();
  ...
  UFUNCTION(BlueprintCallable, meta = (BlueprintInternalUseOnly = "true"),
            Category = "EpicLeaderboard")
  static USubmitLeaderboardEntry *
  SubmitLeaderboardEntry(FEpicLeaderboardGame Game,
                         FEpicLeaderboard Leaderboard, FString Username,
                         double Score) {
    USubmitLeaderboardEntry *BlueprintNode =
        NewObject<USubmitLeaderboardEntry>();
...
protected:
  virtual void Activate() override {
    ...
    // Base URL of the API endpoint
    FString BaseURL =
        FString(TEXT(SERVER_URL)) + FString(TEXT("/api/submitScore"));

    // setup the request
    TSharedRef<IHttpRequest> Request = FHttpModule::Get().CreateRequest();
    Request->OnProcessRequestComplete().BindUObject(
        this, &USubmitLeaderboardEntry::OnScoreSubmitResponseReceived);
  ...
  void OnScoreSubmitResponseReceived(FHttpRequestPtr Request,
                                     FHttpResponsePtr Response,
                                     bool bWasSuccessful) {
    this->RemoveFromRoot();
    ...
    // FString time = Response->GetHeader("X-Response-Time");
    // UE_LOG(LogTemp, Warning, TEXT("SubmitLeaderboardEntry Response Time:
    // %s"), *time);
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(
    FEpicLeaderboardIsUsernameAvailableEvent, IsUsernameAvailableResponse,
    Result);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(
    FEpicLeaderboardIsUsernameAvailableErrorEvent);

UCLASS()
class EPICLEADERBOARD_API UIsUsernameAvailable
    : public UBlueprintAsyncActionBase {
  GENERATED_BODY()
...
public:
  ...
  UFUNCTION(BlueprintCallable, meta = (BlueprintInternalUseOnly = "true"),
            Category = "EpicLeaderboard")
  static UIsUsernameAvailable *IsUsernameAvailable(FEpicLeaderboardGame Game,
                                                   FString Username) {
...
protected:
  virtual void Activate() override {
    ...
    // Base URL of the API endpoint
    FString BaseURL = FString(TEXT(SERVER_URL)) +
                      FString(TEXT("/api/isUsernameAvailable_v2"));
    ...
    TSharedRef<IHttpRequest> Request = FHttpModule::Get().CreateRequest();
    Request->OnProcessRequestComplete().BindUObject(
        this, &UIsUsernameAvailable::OnResultReceived);
  ...
  void OnResultReceived(FHttpRequestPtr Request, FHttpResponsePtr Response,
    ...
    // FString time = Response->GetHeader("X-Response-Time");
    // UE_LOG(LogTemp, Warning, TEXT("UIsUsernameAvailable Response Time: %s"),
    // *time);
